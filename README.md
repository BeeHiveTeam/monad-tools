...
| Tool | What |
|---|---|
| **[monad-doctor](doctor/)** | Pre-flight readiness check — hardware/OS/network/security/monad/VDP. **66 checks** in 30 seconds, JSON output, exits 0/1/2. Extended fix hints (Quick fix / Auto / Verify / Why) on every FAIL & WARN. |
...
Companion repo: [BeeHiveTeam/monad-grafana](https://github.com/BeeHiveTeam/monad-grafana) — Prometheus + Grafana + node_exporter monitoring stack (4 containers, Grafana dashboard, `--public` flag for browser access). Installs in one command.
...
# 2. Check the server is ready (30 seconds)
