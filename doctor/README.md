...
**50 checks across 5 sections** — tells you in 30 seconds whether your server is ready to be a Monad validator, and exactly what to fix if not.
...
| **MONAD** (8) | system user `monad` · `/dev/triedb` symlink · **`monad.triedb-udev`** (`/etc/udev/rules.d/99-triedb.rules` with `SYMLINK+="triedb"` so symlink survives reboot) · **`monad.cruft-timer`** (hourly artifact cleanup, ships with package) · package state (incl. apt hold) · current vs available version · **`monad.upstream`** (GitHub releases/latest, independent of apt cache) · Auth UDP active (wireauth log entries) · 4 Auth UDP keys in `node.toml` in **correct TOML sections** (`[peer_discovery]` and `[network]`) |
